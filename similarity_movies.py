...
def main():
    ...
    for joinQuery, val, dist in joinQueries:
        ...
        # else:
        full_path = "/data/saketh_temp/PSQL1503/Join-Game/Join_Full/"+algorithm
    ...
    for val in vals:
        for dist in lv:
            ...
            # else:
            unweighted_path = "/data/saketh_temp/PSQL1503/Join-Game/Join_Summary/"+algorithm+"/unweighted/"
            weighted_path = "/data/saketh_temp/PSQL1503/Join-Game/Join_Summary/"+algorithm+"/weighted/"
...
def constructQueries(vals, lv):
    result = []
    # limits = ['1234683', '9472693', '4410281']         #2 Relation
    limits = ['1605236', '4304167', '1245818']          #3 relation
    for val in vals:
        for dist in lv:
            # query = ("""SELECT imdb{0}.title, omdbMovies{0}.title 
            #         FROM imdb{0} JOIN omdbMovies{0} 
            #         ON levenshtein(trim(imdb{0}.title::varchar(10)), trim(omdbMovies{0}.title::varchar(10))) <= {1} 
            #         LIMIT {2};""".format(val, dist, limits[int(dist)-6])) 
            query = ("""SELECT sub.title1, sub.title2, actors{0}.knownForTitles
                        FROM (
                            SELECT imdb{0}.title AS title1, omdbMovies{0}.title AS title2
                            FROM imdb{0}
                            JOIN omdbMovies{0} ON levenshtein(trim(imdb{0}.title::varchar(10)), trim(omdbMovies{0}.title::varchar(10))) <= {1}
                        ) sub
                        JOIN actors{0}
                            ON levenshtein(trim(sub.title1::varchar(10)), trim(actors{0}.knownForTitles::varchar(10))) <= {1}
                            AND levenshtein(trim(sub.title2::varchar(10)), trim(actors{0}.knownForTitles::varchar(10))) <= {1}
                        LIMIT {2};""".format(val, dist, limits[int(dist)-6]))  
