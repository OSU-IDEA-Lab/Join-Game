...
def constructQueries(vals, lv):
    ...
    for val in vals:
        for dist in lv:
            query = ("""SELECT imdb{0}.title, omdbMovies{0}.title, actors{0}.knownForTitles 
                                    FROM imdb{0} 
                                    JOIN omdbMovies{0} 
                                    ON levenshtein(trim(imdb{0}.title::varchar(10)), trim(omdbMovies{0}.title::varchar(10))) <= {1} 
                                    JOIN actors{0} 
                                    ON levenshtein(trim(imdb{0}.title::varchar(10)), trim(omdbMovies{0}.title::varchar(10))) <= {1} 
                                    AND levenshtein(trim(omdbMovies{0}.title::varchar(10)), trim(actors{0}.knownForTitles::varchar(10))) <= {1} 
                                    LIMIT 100000;
                                """.format(val, dist))            
