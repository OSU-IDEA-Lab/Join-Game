...
def constructQueries(vals, lv):
    ...
    for val in vals:
        ...
        for dist in lv:
            # query = ("""SELECT Car_brands{0}.make, parking_tickets{0}.vehicle_make, car_accidents{0}.Vehicle_Make 
            #                         FROM Car_brands{0} 
            ...
            #                         ON levenshtein(trim(Car_brands{0}.make::varchar(10)), trim(parking_tickets{0}.vehicle_make::varchar(10))) <= {1} 
            #                         AND levenshtein(trim(parking_tickets{0}.vehicle_make::varchar(10)), trim(car_accidents{0}.Vehicle_Make::varchar(10))) <= {1} 
            #                         LIMIT {2};
            ...
            query = ("""SELECT Car_brands{0}.make, parking_tickets{0}.vehicle_make 
                                        ...
                                        ON levenshtein(trim(Car_brands{0}.make::varchar(10)), trim(parking_tickets{0}.vehicle_make::varchar(10))) <= {1}  
                                        LIMIT {2};
                                    """.format(val, dist, limits[int(dist)-3]))
