...
def main():
    ...
    if len(sys.argv) != 4:
        print("Expecting 3 arguments, verbose output filename and summary filename and algo name")
        exit()
    ...
    for joinQuery, val, dist in joinQueries:
        ...
        cur.execute('set statement_timeout = 18000;')
        
        # Perform the query measurement loop times (currently 1)
        # for i in range(loop):
        print("Running this query for the " + str(1) + " time(s)")
        timeForKs.append(measureTimeForKs(conn, joinQuery, ks, sigma, data_filename+"_Cars", 1))
...
def constructQueries(vals, lv):
    result = []
    limits = ['797181', '1962523', '7313656']
    # limits = ['2226085', '9775307', '1038902']
    for val in vals:
        # Constructs and appends the query, val, and sch_val as a tuple to the result list
        for dist in lv:
            query = ("""SELECT Car_brands{0}.make, parking_tickets{0}.vehicle_make 
                                        FROM Car_brands{0} 
                                        JOIN parking_tickets{0} 
                                        ON levenshtein(trim(Car_brands{0}.make::varchar(10)), trim(parking_tickets{0}.vehicle_make::varchar(10))) <= {1}  
                                        LIMIT {2};
                                    """.format(val, dist, limits[int(dist)-1]))
            # query = ("""SELECT sub.make, sub.vehicle_make, car_accidents{0}.Vehicle_Make
            #             FROM (
            #                 SELECT Car_brands{0}.make, parking_tickets{0}.vehicle_make
            #                 FROM Car_brands{0}
            #                 JOIN parking_tickets{0} ON levenshtein(trim(Car_brands{0}.make::varchar(10)), trim(parking_tickets{0}.vehicle_make::varchar(10))) <= {1}
            #             ) sub
            #             JOIN car_accidents{0}
            #                 ON levenshtein(trim(sub.make::varchar(10)), trim(car_accidents{0}.Vehicle_Make::varchar(10))) <= {1}
            #                 AND levenshtein(trim(sub.vehicle_make::varchar(10)), trim(car_accidents{0}.Vehicle_Make::varchar(10))) <= {1}
            #             LIMIT {2};
            #                         """.format(val, dist, limits[int(dist)-1]))
...
def measureTimeForKs(conn, joinQuery, ks, sigma, data_filename, iteration):
    ...
    try:
        ...
        cur.execute('set from_collapse_limit = 1;') #1800000 = 30 mins
