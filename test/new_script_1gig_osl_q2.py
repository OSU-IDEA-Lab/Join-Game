...
def main():
    ...
    conn = psycopg2.connect(host="/tmp/", database="tpch10g", user="jinjo", port="1531")
    cur = conn.cursor()
    ...
    for joinQuery, val, sch_val in joinQueries:
        ...
        for i in range(loop):
            ...
            timeForKs.append(measureTimeForKs(conn, joinQuery, ks, sigma, data_filename, i, val))
...
def constructQueries(vals):
    result = []
    for val in vals:
        # Explicitly qualify tables with the schema name (z0, z1, or z1_5)
        # This is the most reliable way to benchmark different skew levels
        query = f"""
            ...
            LIMIT 100000;
        """
        # We pass 'val' as the identifier for logging and logic
        result.append((query, val, "1")) 
    return result

def measureTimeForKs(conn, joinQuery, ks, sigma, data_filename, iteration, val):
    res = []
    f = None
    cur = None
    ...
    try:
        ...
        cur.itersize = 1 # Critical for "Time-to-First-K" accuracy
        
        start = time()
        cur.execute(joinQuery) # joinQuery now contains "z0.part", etc.
        ...
        print(f"Query started (Target: z{val}): {joinQuery.strip()[:60]}...")
        ...
        if cur is not None:
            try:
                ...
                pass # Cursor was already closed/invalidated by transaction abort
        if f is not None:
            f.flush()
            f.close()
