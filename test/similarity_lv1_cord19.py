...
def main():
    ...
    loop = 1
    for joinQuery, val in joinQueries:
        timeForKs = []

        summary = open(summary_filename + str(val), 'w+')
    ...
    for val in vals:
        ...
        summary = open(summary_filename + str(val) + ".dat", 'w')
...
def constructQueries(vals):
    ...
    for val in vals:
        ...
        # Q2 - TPCH
        query = ("SELECT fullcord19_1%s.title, fullcord19_2%s.title FROM fullcord19_1%s JOIN fullcord19_2%s ON levenshtein(trim(fullcord19_1%s.title::varchar(10)), trim(fullcord19_2%s.title::varchar(10))) <= 1 LIMIT 10000;" % (val, val, val, val, val, val))
        result.append((query, val))
    return result
