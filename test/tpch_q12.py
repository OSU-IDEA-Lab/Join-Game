...
def main():
    ...
    for size in sizes:
        ...
        cur = conn.cursor()
        summary = open("test/q12_"+size+"g_tpch_"+summary_filename+"_summary.txt", 'w+')

        clear_output = "test/q12_"+size+"g_tpch_"+data_filename+"_output.txt"
...
def constructQueries(vals):
    ...
    for val in vals:
        query = f"""
            ...
            FROM z{val}.orders JOIN z{val}.lineitem 
...
def measureTimeForKs(conn, joinQuery, ks, sigma, data_filename, iteration, val, size):
    ...
    try:
        ...
        # 2. Open log file
        f = open("test/q12_"+size+"g_tpch_"+data_filename+"_output.txt", 'a')
        f.write("======================================================== \n")
