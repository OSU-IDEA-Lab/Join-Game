...
ExecNestLoop(PlanState *pstate)
{
	...
	{	
				...
				if (node->nl_NeedNewInner && !node->ripple_innerEnd)
				{
					...
					{				
						if (!node->ripple_outerEnd)
						{
							...
							node->rippleRightHead = node->rippleRightSize;
						}
		...
		else
		{
			...
			/*
			 * If the tuple failed to join for N times, drop the tuple if N fails.
			 */
			if (outerPlan->LeftReward[node->RLeftHead] == N_FAIL)
			{
				/*
				 * Drop the current slot, then fill the slot with the bottom tuple.
				 * If the pointing slot is not last, move the bottom tuple into the empty slot in the left table.
				...
				{
					TupleTableSlot *replaceTupleSlot;
					replaceTupleSlot = outerPlan->rippleLeftPage[node->rippleLeftSize - 1];

					outerPlan->rippleLeftPage[node->RLeftHead] = MakeSingleTupleTableSlot(replaceTupleSlot->tts_tupleDescriptor);
					ExecCopySlot(outerPlan->rippleLeftPage[node->RLeftHead], replaceTupleSlot);
				...
				/*
				 * After dropping a tuple from the left cache, navigate further process.
				 */
				switch (direction)
				{
					case LEFT_TO_RIGHT:
						...
						{
							/*
							 * If no tuples from the disk,
							 * If no tuples in the left table, stop; otherwise, process for right to left.
							 */							
							if (node->rippleLeftSize == 0)
							{
								return NULL;
						...
						{
							/*
							 * If remaining tuples from the disk,
							 * If no tuples in the left table, keeps process for left to right to build left table.
							 * Otherwise, process for right to left.
							 */									
							if (node->rippleLeftSize == 0)
							{
								node->rippleLeftHead = 0;
					...
					case RIGHT_TO_LEFT:
						...
						{
							/*
							 * If no tuples in the left table and disk, stop; otherwise, get a new inner.
							 */							
							if (node->rippleLeftSize == 0)
							{
								return NULL;
						...
						{
							/*
							 * If the tuple dropped from the left table is last, process for left to right.
							 * Otherwise, do not increment the head because the place is replaced with another.
							 */
							if (node->RLeftHead == node->rippleLeftSize - 1)
							{
								node->rippleRightHead = 0;
