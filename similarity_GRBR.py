...
def main():
    ...
    for joinQuery, val, dist in joinQueries:
        ...
        data = open(data_filename + "_lv" + str(dist) +"_GRBR"+ str(val), 'w+')
    ...
    for val in vals:
        for dist in lv:
            ...
            summary = open(summary_filename + "_lv" + str(dist) +"_GRBR"+ str(val) + ".dat", 'w')
...
def constructQueries(vals, lv):
    ...
    for val in vals:
        ...
        for dist in lv:
            query = ("SELECT gr_Books%s.author_name, gr_Authors%s.name FROM gr_Books%s JOIN gr_Authors%s ON levenshtein(trim(gr_Books%s.author_name::varchar(10)), trim(gr_Authors%s.name::varchar(10))) <= %s LIMIT 175310;" % (val, val, val, val, val, val, dist))
