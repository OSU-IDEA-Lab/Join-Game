...
def main():
    ...
    for joinQuery, val, dist in joinQueries:
        ...
        full_path = "/data/mettas/Join-Game/Join_Full"

        data = open(os.path.join(full_path, data_filename + "_lv" + str(dist) +"_Spotify"+ str(val)), 'w+')
    ...
    for val in vals:
        for dist in lv:
            ...
            avg_unweighted = []
            avg_weighted = []
            unweighted_path = "/data/mettas/Join-Game/Join_Summary/unweighted"
            weighted_path = "/data/mettas/Join-Game/Join_Summary/weighted"

            summary_unweighted = open(os.path.join(unweighted_path, summary_filename + "_lv" + str(dist) +"_Spotify"+ str(val) + "_unweighted.dat"), 'w+')
            summary_weighted = open(os.path.join(weighted_path, summary_filename + "_lv" + str(dist) +"_Spotify"+ str(val) + "_weighted.dat"), 'w+')
...
def constructQueries(vals, lv):
    ...
    for val in vals:
        ...
        for dist in lv:
            query = ("SELECT artists%s.artist_name, spotify_tracks%s.artists FROM artists%s JOIN spotify_tracks%s ON artists%s.artist_name = spotify_tracks%s.artists;" % (val, val, val, val, val, val))
