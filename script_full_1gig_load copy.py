...
def main():
    ...
    shuffles = ['1','2','3']
    # Open connection
    conn = psycopg2.connect(host="/tmp/", database="mettas", user="mettas",
                             port="1997")
    cur = conn.cursor()
...
def makeCopyString(scf, val, table):
    ...
    return "copy %s%s%s from '/postgres/mettas-psql/Data/1gig/1_gig_data/shuffle_data_%s/z%s/%s.tbl' WITH DELIMITER AS '|';" % (
