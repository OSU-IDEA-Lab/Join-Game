...
static void storeTIDsExtended(RelationPage *relationPage, struct tupleRewards *tids, int index, int reward, int pageId){
	int i = 0;
	tids[index].reward = reward;
...
/*
Storing group of tuples from inner and outer relation in a storage in main memory
It helps to optimize the loading time of tuples
*/
...
/*
Check the buffer to find out the worst reward yielding outer group
*/
static int checkWorstOuter(NestLoopState *node){
	int i;
	int worstPageIndex = 0;

	for (i = 0; i < node->activeRelationPages; i++) {
		if (node->tidRewards[i].reward < node->tidRewards[worstPageIndex].reward) {
			worstPageIndex = i;
		}
	}
	return node->tidRewards[worstPageIndex].page_id;
}
...
static TupleTableSlot* ExecBanditJoin(PlanState *pstate) {
	...
	for (;;) {
		if (node->needOuterPage) {
			...
			if (!node->reachedEndOfOuter && node->activeRelationPages < node->sqrtOfInnerPages) { //explore
				
				node->isExploring = true;
				
				node->accOutToExploitInd = 0; 

				node->pageIndex++;
				node->pageIndex = MAX(node->pageIndex, node->lastPageIndex);
				
				LoadNextPage(outerPlan, node->outerPage);
				//elog(INFO, "Outer Page %d Loaded", node->outerPageCounter+1);

				// this block stores one group from the outer relation in the main memory
				// it happens if the storage in the main memory has slot for the group
				if(node->outerStorageInd < node->outerStorageSize){
					LoadInStorage(node->outerPage, node->outerRelStorage, node->outerStorageInd, node->outerPageInStore);
					//elog(INFO, "Outer Page %d is stored in the slot %d of the storage", node->outerPageInStore, node->outerStorageInd);
					node->outerStorageInd++;
					node->outerPageInStore++;
				}else{
					// If the storage in the main memory is full we will discard the worst outer group from it 
					// here worst outer group means the group that yielded least rewards during its exploration
					int worstOuterPageInMemory = checkWorstOuter(node);

					int indexOfWorstPage = 0;

					int i;

					for(i = 0; i<node->outerStorageSize; ++i){
						if(node->outerRelStorage[i].page_id == worstOuterPageInMemory){
							indexOfWorstPage = i;
						}
					}

					LoadInStorage(node->outerPage, node->outerRelStorage, indexOfWorstPage, node->outerPageInStore);
					//elog(INFO, "Outer Page %d is stored in the slot %d of the storage", node->outerPageInStore, indexOfWorstPage);
					node->outerPageInStore++;
				}
				...
				if(node->outerPageCounter == 1){
					node->firstEverExplore = true;
				}

				node->lastReward = 0;
				node->exploreStepCounter = 1;
			} else if ((!node->reachedEndOfOuter && node->activeRelationPages == node->sqrtOfInnerPages)
					|| (node->reachedEndOfOuter && node->activeRelationPages > 0)) { // exploit
				
				node->lastPageIndex = MAX(node->pageIndex, node->lastPageIndex);

				// this block handles exploiting multiple outer groups togather
				if(node->groupJoinDone){
					node->outerPage->index = 0;

					node->isExploring = false;
					if(node->groupExploitation == false){
						node->exploitStepCounter = 0;
						node->reachedEndOfInner = true;
					}

					if(node->accOutToExploitInd < node->outerStorageSize){
						node->groupExploitation = true;
						LoadOuterPageFromStorage(outerPlan, node->outerRelStorage, node->outerPage, node->accOutToExploitInd,
							node->ss->ss_currentRelation, node->ss->ss_ScanTupleSlot);

						if(node->accOutToExploitInd == 0){
							node->anotherInnerToExploit = true; 
						}else{
							node->anotherInnerToExploit = false;
						}
						
						node->accOutToExploitInd++;
						node->groupJoinDone = false;
					}else{
						node->groupExploitCounter++;
						//elog(INFO, "Group Exploit Counter %d", node->groupExploitCounter);
						if(node->groupExploitCounter > node->innerPageNumber){
							node->groupExploitation = false;
							//remove all exploited outer pages
							int j, k;

							for(j = 0; j < node->outerStorageSize; ++j){
								int page = node->outerRelStorage[j].page_id;

								for(k = 0; k < node->activeRelationPages; k++){
									if(node->tidRewards[k].page_id == page){
										node->tidRewards[k] = node->tidRewards[node->activeRelationPages - 1];
										node->activeRelationPages--;
										break;
									}
								}
								//elog(INFO, "active relation pages %d", node->activeRelationPages);
							}
							node->outerStorageInd = 0;
							node->groupExploitCounter = 1;
						}else{
							node->accOutToExploitInd = 0;
						}
					}
			...
			if(node->isExploring){
				...
				if(node->groupExploitation == true){
					//node->needOuterPage = true;
					if(node->anotherInnerToExploit == true){
						node->needInnerPage = true;
						//elog(INFO, "It is here, again and again");
						node->anotherInnerToExploit = false;
					}else{
						node->needInnerPage = false;
					}
		...
		if (node->needInnerPage) {
			...
			if(node->isExploring == false){
				LoadNextPage(innerPlan, node->innerPage);
				node->innerTupleCounter += node->innerPage->tupleCount;
				node->innerPageCounter++;
				//elog(INFO, "Loading inner page %d to exploit", node->innerPageCounter);
				node->innerPageCounterTotal++;
			}else{
				// when the first outer group is being explored 
				// all the inner groups it joins with are stored in the main memory
				if(node->firstEverExplore == true){
					LoadNextPage(innerPlan, node->innerPage);
					node->innerTupleCounter += node->innerPage->tupleCount;
					node->innerPageCounter++;
					node->innerPageCounterTotal++;
					//elog(INFO, "Loaded Inner Page %d", node->innerPageCounter);

					if(node->innerStorageInd < node->innerStorageSize){
						node->innerPageInStore = node->innerPageCounter;
						//elog(INFO, "Loading Inner Page %d in the store position %d", node->innerPageInStore, node->innerStorageInd);
						
						LoadInStorage(node->innerPage, node->innerRelStorage, node->innerStorageInd, node->innerPageInStore);
						node->innerStorageInd++;
					}
				}else{
					if(node->exploreStepCounter <= node->innerStorageInd){
						LoadInnerPageFromStorage(innerPlan,  node->innerRelStorage, node->innerPage, node->exploreStepCounter-1, node->ss_in->ss_currentRelation, node->ss_in->ss_ScanTupleSlot);
						//elog(INFO, "Loading Inner Page %d from the storage position %d", node->innerRelStorage[node->exploreStepCounter-1].page_id, node->exploreStepCounter-1);
					}else{
						LoadNextPage(innerPlan, node->innerPage);
						node->innerTupleCounter += node->innerPage->tupleCount;
						node->innerPageCounter++;
						...
						if(node->innerStorageInd < node->innerStorageSize){
							//elog(INFO, "Inner Page Counter: %d", node->innerPageCounter);
							node->innerPageInStore = node->innerPageCounter;
							//elog(INFO, "Loading Inner Page %d in the store position %d", node->innerPageInStore, node->innerStorageInd);
		...
		if (node->innerPage->index == node->innerPage->tupleCount) {
			if (node->outerPage->index < node->outerPage->tupleCount - 1) {
				...
				if (node->isExploring && ((node->lastReward > 0 && node->exploreStepCounter < node->innerPageNumber)
					...
					storeTIDsExtended(node->outerPage, node->tidRewards, node->activeRelationPages, node->reward, node->outerPageCounter);
					node->nFailure = 0;
					node->reward = 0;	//mx
					node->activeRelationPages++;
					node->needOuterPage = true;
					...
					if(node->firstEverExplore == true){
						node->firstEverExplore = false;
					}
				} 
				else if (!node->isExploring && node->groupExploitCounter <= node->innerPageNumber) {
					node->outerPage->index = 0;
					node->reward += node->lastReward;
					node->lastReward = 0;
					node->exploitStepCounter++;
...
//	}
	...
	// variables for main memory outer storage 
	nlstate->outerStorageSize = 50; //storage size
	nlstate->outerRelStorage = palloc(nlstate->outerStorageSize * sizeof(struct tuplePages)); // outer storage
	nlstate->outerStorageInd = 0; //outer storage index
	nlstate->outerPageInStore = 1; //outer page tracker

	// variables for controlling exploitation of multiple outer pages altogather 
	nlstate->groupExploitCounter = 1; 
	nlstate->groupJoinDone = true; // track group exploitation 
	nlstate->anotherInnerToExploit = false;
	nlstate->groupExploitation = false; 

	// variables for main memory inner storage 
	nlstate->innerStorageSize = 50; //storage size
	nlstate->innerRelStorage = palloc(nlstate->innerStorageSize * sizeof(struct tuplePages)); //inner storage
	nlstate->innerStorageInd = 0;//inner storage index
	nlstate->innerPageInStore = 1; //inner page tracker

	nlstate->firstEverExplore = false; 
	nlstate->accOutToExploitInd = 0; 
