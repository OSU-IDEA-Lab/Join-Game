...
#include "executor/nodeSeqscan.h" //mmm
...
static int LoadNextPageOne(PlanState* planState, RelationPage* relationPage) {
	int i,j;
//	SeqScanState *seqNode = castNode(SeqScanState, planState);
//	int size = PAGE_SIZE;
	if (relationPage == NULL){
...
//		ExecReScan(outerPlan);
	...
	for (;;)
	{
		if (node->needOuterPage) {
			...
			if ((node->level == 1 && node->activeRelationPages >= node->sqrtOfInnerPages) ||
...
//						elog(INFO,"node level is %d, node reward is %d",node->level,node->reward);
					...
					node->needOuterPage = true;
					elog(INFO,"generatedJoins %d",node->generatedJoins - node->tmpJoins);
					node->tmpJoins = node->generatedJoins;
				} else {
...
//	elog(INFO, "isTop: %ld", nlstate->isTop);
	...
	//if (strcmp(fliporder, "on") == 0) {
	if(nlstate->level == 2){
		nlstate->outerPageNumber = outerPlan(node)->plan_rows / (TIMES*PAGE_SIZE2);
	}else{
...
//	elog(INFO,"outer plan rows is %f,inner plan rows is %f",outerPlan(node)->plan_rows,innerPlan(node)->plan_rows);
	...
	if(nlstate->level == 1){
		...
		nlstate->nn = pow(nlstate->innerPageNumber,0.667) > 1?pow(nlstate->innerPageNumber,0.667)*pow(log(nlstate->innerPageNumber),0.333):1;//*pow(log(nlstate->innerPageNumber),0.333);
//		nlstate->selectivity = nlstate->nn*TIMES*PAGE_SIZE/nlstate->outerPageNumber;
		nlstate->selectivity = 1;
...
//	if (strcmp(fliporder, "on") == 0){
		if(nlstate->level == 2){
			...
			nlstate->outerPage = CreateRelationPageOne(nlstate,(TIMES*PAGE_SIZE2));
		}else {
			nlstate->innerPage = CreateRelationPageOne(nlstate,(TIMES*PAGE_SIZE));
...
//	}
	...
	*/
	...
	if (strcmp(fastjoin, "on") == 0){
		elog(INFO, "Seqscan version - Running bandit join..");
	} else {
...
//			node->outerPage = CreateRelationPageOne(node,2*PAGE_SIZE);
			...
			node->outerPage = CreateRelationPageOne(node,(TIMES*PAGE_SIZE2));
		}else {
			node->innerPage = CreateRelationPageOne(node,(TIMES*PAGE_SIZE));
