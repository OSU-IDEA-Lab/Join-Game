...
static int LoadPageWithIndex(PlanState *plan, RelationPage *relationPage, ScanKey xidScanKey, int fromIndex,
		int scanAttribute) {
	int i, j = 0;
	TupleTableSlot *tts;
...
//	fromXid = fromIndex;
	if (relationPage == NULL) {
		elog(ERROR, "LoadPageWithIndex: null page");
	}
	relationPage->index = 0;
	relationPage->tupleCount = 0;
	// Remove the old stored tuples
	...
	for (i = 0; i < PAGE_SIZE; i++) {
		while (true) {
			ScanKeyEntryInitialize(xidScanKey, //TODO is it fine to init ScanKey once?
					0, // flags
					scanAttribute, /* attribute number to scan */
					3, /* op's strategy */
					23, /* strategy subtype */
					0, // ((OpExpr *) clause)->inputcollid,	/* collation */
					65, /* reg proc to use */
			...
			j++;
			if (IsA(plan, IndexScanState)) {
				((IndexScanState*) plan)->iss_NumScanKeys = 1;
				((IndexScanState*) plan)->iss_ScanKeys = xidScanKey;
			} else if (IsA(plan, IndexOnlyScanState)) {
				((IndexOnlyScanState*) plan)->ioss_NumScanKeys = 1;
				((IndexOnlyScanState*) plan)->ioss_ScanKeys = xidScanKey;
			} else {
				elog(ERROR, "Plan type is not index scan");
			}
			ExecReScan(plan);
			tts = ExecProcNode(plan);
			if (TupIsNull(tts)) {
				continue;
				// relationPage->tuples[i] = NULL;
				// break;
...
static int popBestOuterPageXid(NestLoopState *node) {
	int i;
	int bestPageIndex;
	int bestXid;

	bestPageIndex = 0;
	for (i = 0; i < node->activeOuterRelationPages; i++) {
		if (node->rewardsOuter[i] > node->rewardsOuter[bestPageIndex]) {
			bestPageIndex = i;
		}
	}
	bestXid = node->xidsOuter[bestPageIndex];
	node->xidsOuter[bestPageIndex] = node->xidsOuter[node->activeOuterRelationPages - 1];
	node->rewardsOuter[bestPageIndex] = node->rewardsOuter[node->activeOuterRelationPages - 1];
	node->activeOuterRelationPages--;
	return bestXid;
}

static int popBestPageXid(NestLoopState *node) {
	int i;
	int bestPageOuterIndex;
	int bestPageInnerIndex;
	int bestXid;

	bestPageOuterIndex = 0;
	bestPageInnerIndex = 0;
	elog(INFO, "active outer: %d", node->activeOuterRelationPages);
	for (i = 0; i < node->activeOuterRelationPages; i++) {
		if (node->rewardsOuter[i] > node->rewardsOuter[bestPageOuterIndex]) {
			bestPageOuterIndex = i;
		}
	}

	for (i = 0; i < node->activeInnerRelationPages; i++) {
		if (node->rewardsInner[i] > node->rewardsInner[bestPageInnerIndex]) {
			bestPageInnerIndex = i;
		}
	}
	if (node->activeInnerRelationPages != 0
			&& node->rewardsInner[bestPageInnerIndex] > node->rewardsOuter[bestPageOuterIndex]) {
		node->isBestPageOuter = false;
		bestXid = node->xidsInner[bestPageInnerIndex];
		node->xidsInner[bestPageInnerIndex] = node->xidsInner[node->activeInnerRelationPages - 1];
		node->rewardsInner[bestPageInnerIndex] = node->rewardsInner[node->activeInnerRelationPages - 1];
		node->activeInnerRelationPages--;
	} else {
		node->isBestPageOuter = true;
		bestXid = node->xidsOuter[bestPageOuterIndex];
		node->xidsOuter[bestPageOuterIndex] = node->xidsOuter[node->activeOuterRelationPages - 1];
		node->rewardsOuter[bestPageOuterIndex] = node->rewardsOuter[node->activeOuterRelationPages - 1];
		node->activeOuterRelationPages--;
	}
	elog(INFO, "active outer: %d", node->activeOuterRelationPages);
	elog(INFO, "Best Outer Page Index: %d", bestPageOuterIndex);
	elog(INFO, "Best Outer Reward: %d", node->rewardsOuter[bestPageOuterIndex]);
	elog(INFO, "Best Inner Page Index: %d", bestPageInnerIndex);
	return bestXid;
}
...
static void PrintNodeCounters(NestLoopState *node) {
	...
	elog(INFO, "Active Relations: %d", node->activeOuterRelationPages);
...
static TupleTableSlot* ExecRightBanditJoin(PlanState *pstate) {
	...
	for (;;) {
		if (node->needOuterPage) {
			if (!node->reachedEndOfOuter && node->activeOuterRelationPages < node->sqrtOfInnerPages) {
				// explore
				node->isExploringOuter = true;
				node->pageIndex++;
				node->pageIndex = MAX(node->pageIndex, node->lastPageIndex);
				LoadPageWithIndex(outerPlan, node->outerPage, node->xidScanKeyOuter, node->pageIndex, 1);
			...
			} else if ((!node->reachedEndOfOuter && node->activeOuterRelationPages == node->sqrtOfInnerPages)
					|| (node->reachedEndOfOuter && node->activeOuterRelationPages > 0)) {
				// exploit
				...
				node->pageIndex = popBestOuterPageXid(node);
				LoadPageWithIndex(outerPlan, node->outerPage, node->xidScanKeyOuter, node->pageIndex, 1);
			} else {
				// join is done
		...
		if (node->innerPage->index == node->innerPage->tupleCount) {
			if (node->outerPage->index < node->outerPage->tupleCount - 1) {
				...
				if (node->isExploringOuter && node->lastReward > 0
					...
					node->xidsOuter[node->activeOuterRelationPages] = node->pageIndex;
					node->rewardsOuter[node->activeOuterRelationPages] = node->reward;
					node->activeOuterRelationPages++;
					node->needOuterPage = true;
		...
		if (TupIsNull(outerTupleSlot)) {
			if (node->activeOuterRelationPages > 0) { // still has pages in stack
	...
	for (;;) {
		if (node->needOuterPage) {
			if (node->outerTurn) {
				...
				if (!node->reachedEndOfOuter && node->activeOuterRelationPages < node->sqrtOfInnerPages) {
					// explore
					elog(INFO, "sqrtOfInnerPages: %d", node->sqrtOfInnerPages);
					node->isExploringOuter = true;
					node->outerPageCounter++;
					// node->pageIndex = MAX(node->pageIndex, node->lastPageIndex);
					node->outerStartKeyValue = node->outerEndKeyValue;
					node->outerEndKeyValue += LoadPageWithIndex(outerPlan, node->outerPage, node->xidScanKeyOuter,
							node->outerEndKeyValue, 1);
					...
					elog(INFO, "sqrtOfInnerPages: %d", node->sqrtOfInnerPages);
					// exploit
					elog(INFO, "active outer 1: %d", node->activeOuterRelationPages);
					node->outerPage->index = 0;
					node->isExploringOuter = false;
					node->exploitOuterStepCounter = 0;
					// node->lastPageIndex = MAX(node->pageIndex, node->lastPageIndex);
					...
					if (node->isBestPageOuter) {
						...
						LoadPageWithIndex(outerPlan, node->outerPage, node->xidScanKeyOuter, node->pageIndex, 1);
						elog(INFO, "Best PAge Outer loaded");
						node->needOuterPage = false;
						node->needInnerPage = true;
					} else {
						elog(INFO, "Best PAge Inner to be loaded");
						LoadPageWithIndex(innerPlan, node->innerPage, node->xidScanKeyInner, node->pageIndex, 1);
		...
		if (node->needInnerPage) {
			if (node->outerTurn) {
				...
				if (!node->reachedEndOfInner && node->activeInnerRelationPages < node->sqrtOfOuterPages) {
					// explore
					node->isExploringInner = true;
					node->innerPageCounter++;
					// node->pageIndex = MAX(node->pageIndex, node->lastPageIndex);
					node->innerStartKeyValue = node->innerEndKeyValue;
					node->innerEndKeyValue += LoadPageWithIndex(innerPlan, node->innerPage, node->xidScanKeyOuter,
							node->innerEndKeyValue, 1);
					...
					node->needInnerPage = false;
					node->needOuterPage = true;
				} else if ((!node->reachedEndOfInner && node->activeInnerRelationPages == node->sqrtOfOuterPages)
						|| (node->reachedEndOfInner && node->activeInnerRelationPages > 0)) {
					// exploit
					node->innerPage->index = 0;
		...
		if (node->outerTurn && node->innerPage->index == node->innerPage->tupleCount) {
			if (node->outerPage->index < node->outerPage->tupleCount - 1) {
				...
				if (node->isExploringOuter && node->lastReward > 0
					...
					//push the current explored page
					node->xidsOuter[node->activeOuterRelationPages] = node->outerStartKeyValue;
					node->rewardsOuter[node->activeOuterRelationPages] = node->reward;
					node->reward = 0;
					node->activeOuterRelationPages++;
					node->outerTurn = false;
					node->needOuterPage = true;
					elog(INFO, "Last Reward: 0");
				} else if (!node->isExploringOuter && node->exploitOuterStepCounter < node->innerPageNumber) {
					node->outerPage->index = 0;
					node->exploitOuterStepCounter++;
				} else if (!node->isExploringOuter && node->exploitOuterStepCounter == node->innerPageNumber) {
		...
		if (!node->outerTurn && node->outerPage->index == node->outerPage->tupleCount) {
			if (node->innerPage->index < node->innerPage->tupleCount - 1) {
				...
				node->needOuterPage = true;
				if (node->isExploringInner && node->lastReward > 0
						&& node->exploreInnerStepCounter < node->outerPageNumber) { //stay with current
					node->innerPage->index = 0;
					node->reward += node->lastReward;
					node->lastReward = 0;
					node->exploreInnerStepCounter++;
					elog(INFO, "inner reward %d", node->reward);
				} else if (node->isExploringInner && node->exploreInnerStepCounter == node->outerPageNumber) {
					// we have generated all possible joins for the current output page
					// while exploring, no need to store it
					node->needInnerPage = true;
				} else if (node->isExploringInner && node->lastReward == 0) {
					//push the current explored page
					node->xidsInner[node->activeInnerRelationPages] = node->innerStartKeyValue;
					node->rewardsInner[node->activeInnerRelationPages] = node->reward;
					node->reward = 0;
					node->activeInnerRelationPages++;
					node->outerTurn = true;
					node->needInnerPage = true;
		...
		if (node->outerTurn) {
			...
			if (TupIsNull(innerTupleSlot)) {
				if (node->activeInnerRelationPages > 0) { // still has pages in stack
					// elog(WARNING, "Finishing join while there are active pages");
...
ExecInitNestLoop(NestLoop *node, EState *estate, int eflags) {
	...
	nlstate->activeOuterRelationPages = 0;
	...
	nlstate->innerStartKeyValue = 1;
	nlstate->innerEndKeyValue = nlstate->innerStartKeyValue;

	if (strcmp(fliporder, "on") == 0) {
		nlstate->outerPageNumber = innerPlan(node)->plan_rows / PAGE_SIZE + 1;
		// nlstate->innerPageNumber = outerPlan(node)->plan_rows / PAGE_SIZE + 1;
		nlstate->innerPageNumber = 6001215 / PAGE_SIZE + 1;
		// nlstate->innerPageNumber = 25 / PAGE_SIZE + 1;
	} else {
		nlstate->outerPageNumber = outerPlan(node)->plan_rows / PAGE_SIZE;
		// nlstate->innerPageNumber = innerPlan(node)->plan_rows / PAGE_SIZE;
		nlstate->innerPageNumber = 6001215 / PAGE_SIZE;
		// nlstate->innerPageNumber = 25 / PAGE_SIZE;
	}
	...
	nlstate->xidsOuter = palloc(nlstate->sqrtOfInnerPages * sizeof(int));
	nlstate->rewardsOuter = palloc(nlstate->sqrtOfInnerPages * sizeof(int));

	nlstate->sqrtOfOuterPages = (int) sqrt(nlstate->outerPageNumber);
	nlstate->xidsInner = palloc(nlstate->sqrtOfOuterPages * sizeof(int));
	nlstate->rewardsInner = palloc(nlstate->sqrtOfOuterPages * sizeof(int));

	nlstate->pageIndex = -1;
	nlstate->lastPageIndex = 0;
	nlstate->xidScanKeyOuter = (ScanKey) palloc(sizeof(ScanKeyData));
	nlstate->xidScanKeyInner = (ScanKey) palloc(sizeof(ScanKeyData));
...
//	}
	...
	pfree(node->xidsOuter);
	pfree(node->rewardsOuter);
	pfree(node->xidsInner);
	pfree(node->rewardsInner);
	pfree(node->xidScanKeyOuter);
