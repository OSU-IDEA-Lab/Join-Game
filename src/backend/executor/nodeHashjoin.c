	...
	for (;;)
	{
		...
		switch (node->hj_JoinState)
		{
			...
			case HJ_EHJ_SYMMETRIC:
			{
				...
				{
					/*
					 * Phase 1 complete.  Destroy the EHJ table and return
					 * NULL to signal end of join.
					 *
					 * We do NOT fall through to HJ_BUILD_HASHTABLE here.
					 * That path would rescan both relations and re-emit all
					 * matches, producing duplicates for every tuple that was
					 * already output during the symmetric phase.
					 *
					 * Phase 2 (biased flush / spill) will eventually replace
					 * this return with logic to continue processing the
					 * remaining tuples that did not fit in memory.
					 */
...
ExecInitHashJoin(HashJoin *node, EState *estate, int eflags)
{
	...
	/*
	 * EHJ: start in the symmetric phase.  The HJ_BUILD_HASHTABLE state is
	 * still reachable as a fallback when Phase 1 ends (memory full or both
	 * sources exhausted) and is used to rebuild for standard batch processing.
	 */
