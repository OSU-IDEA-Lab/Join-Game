...
METHODS = ["simple", "middle", "complete"]
COLORS  = {"simple": "blue", "middle": "green", "complete": "red"}
LABELS  = {"simple": "with multiplier", "middle": "without multiplier", "complete": "with multiplier and size of relation multiplied"}
...
def plot_means_combined(means_final, out_path, trim=0, title="Mean Error % for Cars Full"):
    ...
    for method in METHODS:
        ...
        if method == "complete" and len(items) > 10:
            items = items[10:]
        elif method == "simple" and len(items) > 10:
            items = items[100:]
        elif method == "middle" and len(items) > 10:
            items = items[3:]
...
def main(argv=None):
    ...
    parser.add_argument("files", nargs=9, help="Nine log files in order: simple1 simple2 simple3 middle1 middle2 middle3 complete1 complete2 complete3")
    ...
    plot_means_combined(means_final, outdir / "mean_error_combined.png", trim=args.trim,
                        title="Mean Error % for Cars Full")
    for m in METHODS:
